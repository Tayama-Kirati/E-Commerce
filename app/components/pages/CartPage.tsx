...
export function CartPage() {
 ...
 {cartItems.length === 0 ? (
 <div className="text-center py-24"><div className="text-7xl mb-5">🛒</div><h2 className="text-2xl font-black text-gray-900 dark:text-white mb-2">Your cart is empty</h2><p className="text-gray-400 mb-8">Add some amazing products!</p><button onClick={() => nav("home")} className="px-8 py-3.5 bg-orange-500 text-white font-black rounded-2xl hover:bg-orange-600 transition-colors">Start Shopping</button></div>
 ) : (
 <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6">
 <div className="bg-background dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden">
 ...
 <button onClick={() => updateQty(item.id, item.qty-1)} className="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-blue-600 font-bold text-lg transition-colors">−</button>
 <span className="w-8 text-center font-black text-gray-900 dark:text-white">{item.qty}</span>
 <button onClick={() => updateQty(item.id, item.qty+1)} disabled={item.qty>=(item.stock??99)} className="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-blue-600 font-bold text-lg transition-colors disabled:opacity-30">+</button>
 </div>
 <div className="flex items-center gap-4">
 <span className="font-black text-blue-600">{formatPrice(Number(item.basePrice??item.price??0) * item.qty)}</span>
 ...
 <div className="border-t border-gray-100 dark:border-gray-800 pt-2.5 flex justify-between font-black text-lg"><span className="dark:text-white">Total</span><span className="text-blue-600">{formatPrice(cartGrand)}</span></div>
 </div>
 {user ? (
 <button onClick={() => nav("checkout")} className="w-full py-4 bg-orange-500 hover:bg-orange-600 text-white font-black rounded-2xl transition-all active:scale-[0.98] mb-3">Proceed to Checkout →</button>
 ) : (
 <button onClick={() => nav("role-select")} className="w-full py-4 bg-blue-600 text-white font-black rounded-2xl hover:bg-blue-700 transition-colors mb-3">Sign In to Checkout</button>
 )}
 <button onClick={() => nav("home")} className="w-full text-sm text-blue-600 font-semibold hover:underline">Continue Shopping</button>
 </div>
 </div>
 )}
 </div>
